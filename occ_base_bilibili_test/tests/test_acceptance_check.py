...
class TestOccBaseBilibiliAcceptanceCheck(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.Check = cls.env["occ.base.bilibili.acceptance.check"]
        cls.regular_user = new_test_user(
            cls.env,
            login="occ_base_bilibili_acceptance_regular",
            groups="base.group_user",
        )
        cls.system_user = new_test_user(
            cls.env,
            login="occ_base_bilibili_acceptance_system",
    ...
    def test_samples_cover_the_generic_acceptance_scope(self):
        samples = self.Check.browse(
            [
                self.env.ref(f"occ_base_bilibili_test.{xmlid}").id
    ...
    def test_statuses_and_sanitized_notes_are_writable(self):
        check = self.env.ref("occ_base_bilibili_test.check_core_flow")
    ...
    def test_acl_is_granted_only_to_system_administrators(self):
        access = self.env.ref(
            "occ_base_bilibili_test.access_occ_base_bilibili_acceptance_check_system"
        )
    ...
    def test_menu_and_action_are_connected(self):
        root_menu = self.env.ref("occ_base_bilibili_test.menu_acceptance_root")
        menu = self.env.ref("occ_base_bilibili_test.menu_acceptance_check")
        action = self.env.ref("occ_base_bilibili_test.action_acceptance_check")
        ...
        self.assertEqual(action.res_model, "occ.base.bilibili.acceptance.check")
