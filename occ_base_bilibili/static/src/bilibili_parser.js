...
export function parseBilibiliVideo(value, { allowBareBvid = false } = {}) {
    ...
    if (allowBareBvid && BVID_RE.test(candidate)) {
        return normalizedResult(candidate, 1);
    }
...
export function isBilibiliVideoInput(value, { allowBareBvid = false } = {}) {
    if (typeof value !== "string") {
        return false;
    }
    let candidate = value.trim();
    if (/^av[1-9][0-9]*$/i.test(candidate) || (allowBareBvid && BVID_RE.test(candidate))) {
        return true;
    }
